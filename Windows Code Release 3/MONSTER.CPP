...
short monst_pick_target(short which_m)
{
	...
	if (monst_target[which_m] < 6) 
		if (adven[monst_target[which_m]].main_status != 1)
			monst_target[which_m] = 6;		
...
short monst_pick_target_pc(short m_num,creature_data_type *which_m)
{
	...
	r1 = get_ran(1,0,5);
	while ((num_tries < 4) && ((adven[r1].main_status != 1) ||
...
{	
	...
	if (monst_can_be_there(destination,which) == FALSE)
		...
		return FALSE;
				...
				//	print_nums(999, c_town.monst.dudes[which].m_loc.x,c_town.monst.dudes[which].m_loc.y);
				 //	print_nums(monst_target[which],destination.x,destination.y);
				 //	}
...
// Also used for sleep and paralyze, which_statys is 0 means charm
{
	...
	if ((which_status == 11) && (which_m->number >= 138) && (which_m->number <= 142))
		return;
	if ((which_status == 11) && (which_m->m_d.m_type == 8))
		return;
	r1 = get_ran(1,0,100);
	if (which_m->m_d.immunities & 1)
		r1 = r1 * 2;
